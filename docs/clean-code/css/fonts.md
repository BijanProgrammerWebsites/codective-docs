...
## روش صحیح استفاده از `clamp` برای `font-size`

همیشه به این شکل از `clamp` برای `font-size` استفاده کنید:

```
clamp(min, ${some number}vh + 1rem, max)
```

اینجوری اگه کاربر Zoom In یا Zoom Out کنه، سایز فونت همراه با صفحه کوچیک و بزرگ میشه.
