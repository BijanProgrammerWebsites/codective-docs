...
برای اینکه کاربردش جا بیفته، ساختمان داده‌ی Stack رو با استفاده از TypeScript پیاده‌سازی می‌کنیم.

پشته یا Stack یه ساختمان داده‌ست که دو تا متد اصلی به نام‌های `push` و `pop` داره.
اگه از push استفاده کنیم، آیتم مورد نظر رو می‌تونه به انتهای پشته اضافه کنیم.
اگه از `pop` استفاده کردیم، از انتهای پشته یه آیتم برمی‌داریم.

مثالش تو دنیای واقعی شبیه به پشته‌ای کتاب‌هاست که روی هم قرار داده شده.
اگه بخوایم کتاب جدیدی اضافه کنیم باید بذاریم روی بقیه‌ی کتاب‌ها.
اگه بخوایم کتابی برداریم، باید از رو برداریم؛ نمی‌تونیم از وسط یا زیر برداریم.
